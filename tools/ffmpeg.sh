##############################################################################
# FFMPEG CHEATSHEET (中文速查表) - by GaoFei (created on 2023/03/01)
##############################################################################
...
ffmpeg -i input.mp4 -an -vcodec copy -bsf:v \
       h264_mp4toannexb output.h264                         # 提取 h264
ffmpeg -i input.mp4 -vcodec libx264 -x264opts \
       "bframes=0" output.h264                              # 移除 B 桢
...
ffmpeg -ss 0 -t 120 -i abc.mp3 -f s16le -acodec pcm_s16le -b:a 16 \
       -ar 48000 -ac 2 abc.pcm                              # 提取PCM数据

##############################################################################
# 推流
##############################################################################

ffmpeg -re -i cap.h264 -vcodec copy -f rtp rtp://ip:port -vn \
       -acodec copy -f rtp rtp://ip:port                    # 推RTP流